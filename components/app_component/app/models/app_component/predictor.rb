module AppComponent
  class Predictor
    ...
    def learn(games)
      games.each do |game|
        ...
        if game.winning_team == 1
          game_result = [first_team_rating, second_team_rating]
        else
          game_result = [second_team_rating, first_team_rating]
        end
    ...
    def higher_mean_team(first_team, second_team)
      first_team_mean = @teams_lookup[first_team.id][:rating].first.mean
      second_team_mean = @teams_lookup[second_team.id][:rating].first.mean

      first_team_mean > second_team_mean
    end
  end
end
